...
### 1.a) MOTD Script for General VMs
```bash
#!/bin/bash
...
### 1.b) MOTD Script for Laptop - Includes Battery Power Details
```bash
#!/bin/bash
...
item "CPU usage" "$LOAD (1, 5, 15 min)"
item "Battery state" "${BAT_STATE}"
item "Battery percentage" "${BAT_PERCENTAGE}"
item "Memory used" "${MEM_USED} MB / ${MEM_TOTAL} MB"
item "Swap in use" "${SWAP_USED} MB"
item "System uptime" "$UPTIME"
item "Disk space Root" "${ROOT_FREE} remaining"
